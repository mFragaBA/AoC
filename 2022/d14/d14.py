...
"""
Submition for Advent of Code 2022 - Day 14
...
RENDER=ON python3 d14.py
```

## Resources

I used these resources to solve this (most if not all are to render the map and shouldn't be necessary)
...
class World:
    def __init__(self, paths) -> None:
        self.minx = min(min([min([p[0] for p in l]) for l in paths]), 500)
        self.maxx = max(max([max([p[0] for p in l]) for l in paths]), 500)
    ...
    def try_step(self):
        ...
        if self.out_of_bounds(down[0], down[1]):
        ...
        if self.out_of_bounds(left_down[0], left_down[1]):
            return StepResult.END
        ...
        if self.out_of_bounds(right_down[0], right_down[1]):
            return StepResult.END
    ...
    def draw(self):
        ...
        for y in range(start_y, end_y + 1):
            for xr in range(self.minx, self.maxx + 1):
                x = xr - self.minx
