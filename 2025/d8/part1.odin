...
kruskal :: proc(edges: [dynamic]Edge, num_points: int, num_steps: int) -> [dynamic]int {
	...
	for i := 0; i < num_steps; i += 1 {
		...
		if uf_find(&uf, u.id) == uf_find(&uf, v.id) {
			fmt.printfln(
				"Connecting boxes at (%i, %i, %i) and (%i, %i, %i) at distance, %i... but nothing happens",
				u.x,
				u.y,
				u.z,
				v.x,
...
main :: proc() {
	...
	uf_count := kruskal(edges, len(nodes), 1000)

	sort.quick_sort(uf_count[:])

	fmt.println(uf_count)

	largest := uf_count[len(uf_count) - 1]
	second_largest := uf_count[len(uf_count) - 2]
	third_largest := uf_count[len(uf_count) - 3]
	fmt.println(largest * second_largest * third_largest)
}
